...
const EndpointModal: React.FC<EndpointModalProps> = ({ isOpen, onClose, editEndpointId }) => {
  ...
  const fetchModels = async () => {
    ...
    try {
      ...
      const response = await fetch('http://172.187.232.176:5000/api/fetch-models/', {
        method: 'POST',
        headers: {
      ...
      if (response.ok && data.success) {
        ...
        toast({
          ...
          description: data.error || "Could not fetch models from the endpoint. Using default models list.",
          variant: "destructive"
        });
      }
    } catch (error) {
      ...
      toast({
        title: "Error fetching models",
        description: "Could not connect to the server. Using default models list.",
        variant: "destructive"
      });
    } finally {
